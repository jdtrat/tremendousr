...
Given the most likely reason for using tremendousr is to send rewards, we have a nice helper function to do just that! With your Tremendous Client created, it's as simple as calling `trem_send_reward()` with the proper arguments. A simple example is below. For more documentation on each argument, please call `?trem_send_reward()` in your console.
